...
http.createServer((req, res) => {
  ...
  if (req.url.substring(0, 5) === "/api/" && req.url.length > 5){
    ...
    // foundbeatle es el beatle que coincide con aquel que dio true
    let foundBeatle = beatles.find((b) => findBeatle === encodeURI(b.name)); // es booleano
    if (foundBeatle) {
      res.writeHead(200, {"Content-type": "application/json"});
  ...
  if (foundBeatle) {
    ...
    // Una alternativa es remplazar cada valro por separado
